...
int32_t main(int32_t argc, char **argv) {
    ...
    // output samples
    LweSamplelvl2 *test_out = new_LweSamplelvl2_array(nb_samples, in_out_params);
    ...
    clock_t begin = clock();
    for (int32_t i = 0; i < nb_samples; ++i) {
        tfhe_bootstraplvl02_woKS_FFT(test_out + i, keyset->cloud.bkFFT, mu_boot, test_in + i);
    }
    clock_t end = clock();
    ...
    for (int32_t i = 0; i < nb_samples; ++i) {
        Torus64 phase = lwelvl2Phase(test_out + i, &keyset->tgsw_key->tlwe_key);
        cout << "phase " << i << " = " << t32tod(phase) << endl;
    }

    delete_LweSamplelvl2_array(nb_samples, test_out);
